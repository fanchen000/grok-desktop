...
| `grok-usage-manager.mjs` | Reads authoritative Grok subscription usage through a hidden official `/usage` TUI session, caches it per account, and removes the probe session without exposing credentials |
...
- Per-account Grok subscription usage retrieval through the official CLI, exposed to local packaged UI only through privileged IPC.
