...
import { createGrokOpenCodeBridge } from '../../scripts/grok-opencode-bridge.mjs';
import { createGrokAccountManager } from './grok-account-manager.mjs';
import { createGrokHistoryManager } from './grok-history-manager.mjs';
import { createGrokUsageManager } from './grok-usage-manager.mjs';
...
const PACKAGED_APP_USER_MODEL_ID = 'ai.x.grokcode.desktop';
const DEV_APP_USER_MODEL_ID = 'ai.x.grokcode.desktop.dev';
...
const GROK_CODE_MODE = process.env.GROK_CODE_MODE !== '0';
const DEFAULT_GROK_BINARY = process.platform === 'win32' ? 'grok.exe' : 'grok';
...
// This fork keeps OpenChamber's mature desktop shell and uses Grok Build ACP
// as the coding engine.
app.setName('Grok Code');
if (process.platform === 'linux') {
  app.setDesktopName('openchamber.desktop');
}
if (isDev) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Grok Code Dev'));
}
...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/fanchen000/grok-desktop/main/CHANGELOG.md';
const GITHUB_BUG_REPORT_URL = 'https://github.com/fanchen000/grok-desktop/issues/new';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/fanchen000/grok-desktop/issues/new';
...
const state = {
  ...
  grokHistoryManager: null,
  grokUsageManager: null,
  sidecarUrl: null,
  localOrigin: null,
  apiBaseUrl: null,
  clientToken: null,
  ...
  activeBrowserWebContentsId: null,
  browserRecording: null,
  browserReplayInProgress: false,
};

const BROWSER_RECORDING_MAX_EVENTS = 500;
const BROWSER_READ_PAGE_DEFAULT_MAX_CHARS = 20_000;

const requireGrokAccountManager = () => {
  if (!GROK_CODE_MODE || !state.grokAccountManager) {
    throw new Error('Grok 账号管理器尚未就绪');
  }
  return state.grokAccountManager;
};

const requireGrokHistoryManager = () => {
  if (!GROK_CODE_MODE || !state.grokHistoryManager) {
    throw new Error('Grok 历史管理器尚未启动');
  }
  return state.grokHistoryManager;
};

const requireGrokUsageManager = () => {
  if (!GROK_CODE_MODE || !state.grokUsageManager) {
    throw new Error('Grok 额度管理器尚未启动');
  }
  return state.grokUsageManager;
};
const BROWSER_READ_PAGE_MAX_CHARS = 50_000;

const browserDataDirectory = (kind) => path.join(app.getPath('userData'), kind);

const getActiveBrowserWebContents = () => {
  const id = state.activeBrowserWebContentsId;
  if (!Number.isInteger(id)) return null;
  const contents = webContents.fromId(id);
  if (!contents || contents.isDestroyed() || contents.getType() !== 'webview') {
    state.activeBrowserWebContentsId = null;
    return null;
  }
  return contents;
};

const requireActiveBrowserWebContents = () => {
  const contents = getActiveBrowserWebContents();
  if (!contents) {
    throw new Error('请先在 Grok Code 右侧面板打开一个浏览器标签页。');
  }
  return contents;
};

const normalizeControllableBrowserUrl = (rawUrl) => {
  const value = String(rawUrl || '').trim();
  if (!value) throw new Error('浏览器 URL 不能为空。');
  const url = new URL(value.includes('://') ? value : `https://${value}`);
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new Error('Computer Use 只允许 HTTP 和 HTTPS 页面。');
  }
  return url.toString();
};

const readBrowserRecordings = async () => {
  const directory = browserDataDirectory('browser-replays');
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    try {
      const payload = JSON.parse(await fsp.readFile(path.join(directory, entry.name), 'utf8'));
      if (!payload || typeof payload !== 'object' || !Array.isArray(payload.events)) continue;
      recordings.push({
        id: String(payload.id || entry.name.slice(0, -5)),
        label: String(payload.label || ''),
        startedAt: Number(payload.startedAt) || 0,
        stoppedAt: Number(payload.stoppedAt) || 0,
        eventCount: payload.events.length,
      });
    } catch {
    }
  }
  return recordings.sort((a, b) => b.startedAt - a.startedAt).slice(0, 50);
};

const appendBrowserRecordingEvent = (name, args, result = null) => {
  const recording = state.browserRecording;
  if (!recording || state.browserReplayInProgress) return;
  if (recording.events.length >= BROWSER_RECORDING_MAX_EVENTS) return;
  const normalizedArgs = args && typeof args === 'object' ? args : {};
  const previous = recording.events[recording.events.length - 1];
  const atMs = Date.now() - recording.startedAt;
  if (
    previous
    && previous.name === name
    && atMs - previous.atMs < 1_000
    && JSON.stringify(previous.arguments) === JSON.stringify(normalizedArgs)
  ) {
    return;
  }
  recording.events.push({
    ...
    atMs,
    pageUrl: typeof result?.url === 'string' ? result.url : undefined,
  });
};

const browserPageSnapshotScript = (maxCharacters) => `(() => {
  const maxCharacters = ${JSON.stringify(maxCharacters)};
  ...
  const entries = [...document.querySelectorAll('a,button,input,textarea,select,[role="button"],[contenteditable="true"],h1,h2,h3')]
    .filter(visible)
    .slice(0, 250)
    .map((element) => ({
      ...
      text: (element.innerText || element.value || element.getAttribute('aria-label') || element.getAttribute('placeholder') || '').trim().slice(0, 500),
  ...
  return {
    ...
    text: (document.body?.innerText || '').trim().slice(0, maxCharacters),
    elements: entries,
  };
})()`;

const invokeBrowserTool = async (name, args = {}, options = {}) => {
  const shouldRecord = options.record !== false;

  if (name === 'browser_status') {
    const contents = getActiveBrowserWebContents();
    const recordings = await readBrowserRecordings();
    if (!contents) {
      return {
        active: false,
        recording: state.browserRecording ? { id: state.browserRecording.id, eventCount: state.browserRecording.events.length } : null,
        recordings,
      };
    }
    const viewport = await contents.executeJavaScript('({ width: innerWidth, height: innerHeight, scrollX, scrollY })', true).catch(() => null);
    return {
      active: true,
      url: contents.getURL(),
      title: contents.getTitle(),
      viewport,
      recording: state.browserRecording ? { id: state.browserRecording.id, label: state.browserRecording.label, eventCount: state.browserRecording.events.length } : null,
      replayInProgress: state.browserReplayInProgress,
      recordings,
    };
  }
  ...
  if (name === 'browser_record_start') {
    if (state.browserRecording) {
      throw new Error('已有浏览器录制正在进行。');
    }
    requireActiveBrowserWebContents();
    state.browserRecording = {
      id: randomUUID(),
      label: typeof args.label === 'string' ? args.label.trim().slice(0, 120) : '',
      startedAt: Date.now(),
      events: [],
    };
    return { id: state.browserRecording.id, startedAt: state.browserRecording.startedAt };
  }

  if (name === 'browser_record_stop') {
    const recording = state.browserRecording;
    if (!recording) throw new Error('当前没有正在进行的浏览器录制。');
    state.browserRecording = null;
    const payload = { ...recording, stoppedAt: Date.now(), version: 1 };
    const directory = browserDataDirectory('browser-replays');
    await fsp.mkdir(directory, { recursive: true });
    const target = path.join(directory, `${recording.id}.json`);
    await fsp.writeFile(target, `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
    return { id: recording.id, eventCount: recording.events.length, path: target };
  }

  if (name === 'browser_replay') {
    const recordingId = String(args.recordingId || '').trim();
    if (!/^[a-zA-Z0-9_-]{8,80}$/.test(recordingId)) throw new Error('无效的录制 ID。');
    if (state.browserReplayInProgress) throw new Error('已有浏览器回放正在进行。');
    const target = path.join(browserDataDirectory('browser-replays'), `${recordingId}.json`);
    const payload = JSON.parse(await fsp.readFile(target, 'utf8'));
    if (!Array.isArray(payload?.events)) throw new Error('录制文件格式无效。');
    state.browserReplayInProgress = true;
    try {
      let previousAt = 0;
      let completed = 0;
      for (const event of payload.events.slice(0, BROWSER_RECORDING_MAX_EVENTS)) {
        const atMs = Number(event?.atMs) || previousAt;
        const delay = Math.max(0, Math.min(1_500, atMs - previousAt));
        if (delay > 0) await new Promise((resolve) => setTimeout(resolve, delay));
        previousAt = atMs;
        if (!['browser_navigate', 'browser_click', 'browser_type', 'browser_scroll'].includes(event?.name)) continue;
        await invokeBrowserTool(event.name, event.arguments ?? {}, { record: false });
        completed += 1;
      }
      return { id: recordingId, completed };
    } finally {
      state.browserReplayInProgress = false;
    }
  }

  const contents = requireActiveBrowserWebContents();

  if (name === 'browser_navigate') {
    const url = normalizeControllableBrowserUrl(args.url);
    await contents.loadURL(url);
    const result = { url: contents.getURL(), title: contents.getTitle() };
    if (shouldRecord) appendBrowserRecordingEvent(name, { url }, result);
    return result;
  }

  if (name === 'browser_read_page') {
    const requested = Number(args.maxCharacters);
    const maxCharacters = Number.isFinite(requested)
      ? Math.max(1_000, Math.min(BROWSER_READ_PAGE_MAX_CHARS, Math.trunc(requested)))
      : BROWSER_READ_PAGE_DEFAULT_MAX_CHARS;
    return await contents.executeJavaScript(browserPageSnapshotScript(maxCharacters), true);
  }

  if (name === 'browser_click') {
    const selector = typeof args.selector === 'string' ? args.selector.trim() : '';
    const text = typeof args.text === 'string' ? args.text.trim() : '';
    if (!selector && !text) throw new Error('browser_click 需要 selector 或 text。');
    const script = `(() => {
      const selector = ${JSON.stringify(selector)};
      const text = ${JSON.stringify(text)};
      let element = null;
      if (selector) {
        try { element = document.querySelector(selector); } catch { return { ok: false, error: 'invalid selector' }; }
      }
      if (!element && text) {
        const candidates = [...document.querySelectorAll('button,a,input[type="button"],input[type="submit"],[role="button"]')];
        element = candidates.find((candidate) => (candidate.innerText || candidate.value || candidate.getAttribute('aria-label') || '').trim() === text)
          || candidates.find((candidate) => (candidate.innerText || candidate.value || candidate.getAttribute('aria-label') || '').includes(text));
      }
      if (!element) return { ok: false, error: 'element not found' };
      element.scrollIntoView({ block: 'center', inline: 'center' });
      element.click();
      return { ok: true, tag: element.tagName.toLowerCase(), text: (element.innerText || element.value || '').trim().slice(0, 300), url: location.href };
    })()`;
    const result = await contents.executeJavaScript(script, true);
    if (!result?.ok) throw new Error(result?.error || '点击失败。');
    if (shouldRecord) appendBrowserRecordingEvent(name, { selector, text }, result);
    return result;
  }
  ...
  if (name === 'browser_type') {
    ...
    if (!selector) throw new Error('browser_type 需要 selector。');
    ...
    const script = `(() => {
      ...
      if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement || element instanceof HTMLSelectElement) {
        element.value = append ? element.value + nextText : nextText;
      } else if (element.isContentEditable) {
        element.textContent = append ? (element.textContent || '') + nextText : nextText;
      } else {
      ...
      return { ok: true, tag: element.tagName.toLowerCase(), valueLength: String(element.value ?? element.textContent ?? '').length, url: location.href };
    })()`;
    const result = await contents.executeJavaScript(script, true);
    if (!result?.ok) throw new Error(result?.error || '输入失败。');
    if (shouldRecord) appendBrowserRecordingEvent(name, { selector, text, append, submit }, result);
    return result;
  }

  if (name === 'browser_scroll') {
    const x = Number.isFinite(Number(args.x)) ? Number(args.x) : 0;
    const y = Number.isFinite(Number(args.y)) ? Number(args.y) : 700;
    const target = ['top', 'bottom'].includes(args.target) ? args.target : 'relative';
    const script = `(() => {
      const target = ${JSON.stringify(target)};
      if (target === 'top') scrollTo({ top: 0, behavior: 'instant' });
      else if (target === 'bottom') scrollTo({ top: document.documentElement.scrollHeight, behavior: 'instant' });
      else scrollBy({ left: ${JSON.stringify(x)}, top: ${JSON.stringify(y)}, behavior: 'instant' });
      return { url: location.href, scrollX, scrollY, documentHeight: document.documentElement.scrollHeight };
    })()`;
    const result = await contents.executeJavaScript(script, true);
    if (shouldRecord) appendBrowserRecordingEvent(name, { x, y, target }, result);
    return result;
  }

  if (name === 'browser_appshot') {
    let image;
    try {
      image = await contents.capturePage();
    } catch (error) {
      if (!/UnknownVizError/i.test(error?.message || String(error))) throw error;
      // Chromium can briefly reject capture while the WebView swaps surfaces
      // after navigation. One bounded retry avoids a false user-visible error
      // without hiding persistent rendering failures.
      await new Promise((resolve) => setTimeout(resolve, 180));
      image = await contents.capturePage();
    }
    const id = randomUUID();
    const directory = browserDataDirectory('browser-appshots');
    await fsp.mkdir(directory, { recursive: true });
    const pngPath = path.join(directory, `${id}.png`);
    ...
    await Promise.all([
      fsp.writeFile(pngPath, image.toPNG()),
      fsp.writeFile(metadataPath, `${JSON.stringify(metadata, null, 2)}\n`, 'utf8'),
    ]);
    return { ...metadata, pngPath, metadataPath };
  }

  throw new Error(`未知浏览器工具：${name}`);
};

const grokBrowserController = {
  invoke: (name, args) => invokeBrowserTool(name, args),
};
...
const quitConfirmationMessage = () => {
  ...
  if (quitRisk.hasActiveTunnel) {
    reasons.push('一个正在运行的远程隧道');
  }
  if (quitRisk.runningScheduledTasksCount > 0) {
    reasons.push(`${quitRisk.runningScheduledTasksCount} 个正在运行的计划任务`);
  }
  if (quitRisk.enabledScheduledTasksCount > 0) {
    reasons.push(`${quitRisk.enabledScheduledTasksCount} 个已启用的计划任务`);
  }
  if (reasons.length === 0) {
    return '退出后将停止后台进程和 SSH 会话。';
  }
  return `Grok Code 检测到${reasons.join('、')}。现在退出会停止后台进程，并可能中断尚未完成的工作。`;
};
...
const shutdownBackgroundServices = () => {
  ...
  if (state.grokBridge) {
    const bridge = state.grokBridge;
    state.grokBridge = null;
    void bridge.close().catch((error) => {
      log.warn('[grok-code] failed to stop Grok ACP bridge:', error);
    });
  }
  if (state.grokUsageManager) {
    const usageManager = state.grokUsageManager;
    state.grokUsageManager = null;
    void usageManager.close().catch((error) => {
      log.warn('[grok-code] failed to stop Grok usage manager:', error);
    });
  }
  if (state.grokAccountManager) {
    const accountManager = state.grokAccountManager;
    state.grokAccountManager = null;
    void accountManager.close().catch((error) => {
      log.warn('[grok-code] failed to stop Grok account manager:', error);
    });
  }
  state.grokHistoryManager = null;
  killSidecar();
  setImmediate(() => {
...
const spawnLocalServer = async () => {
  ...
  if (GROK_CODE_MODE && !state.grokBridge) {
    const grokPath = process.env.GROK_BINARY || DEFAULT_GROK_BINARY;
    if (!state.grokAccountManager) {
      const accountManager = createGrokAccountManager({
        storageRoot: path.join(app.getPath('userData'), 'grok-accounts'),
        grokPath,
        defaultHome: os.homedir(),
        onActiveAccountChanging: async (account, env) => {
          if (state.grokBridge) {
            await state.grokBridge.switchGrokAccount({ accountId: account.id, env });
          }
          state.grokHistoryManager?.invalidate();
          if (state.grokBridge && state.grokHistoryManager) {
            const nativeIndex = await state.grokHistoryManager.list({ limit: 500, force: true });
            await state.grokBridge.syncNativeSessions(nativeIndex.sessions, account.id);
          }
          if (state.grokUsageManager) {
            await state.grokUsageManager.invalidate();
            void state.grokUsageManager.prewarm().catch((error) => {
              log.warn('[grok-code] usage prewarm after account switch failed:', error);
            });
          }
          emitToAllWindows('openchamber:grok-account-changed', {
      ...
      state.grokAccountManager = accountManager;
    }
    if (!state.grokHistoryManager) {
      const usageProbeDirectory = path.join(app.getPath('userData'), 'grok-usage-probe');
      state.grokHistoryManager = createGrokHistoryManager({
        grokPath,
        accountManager: state.grokAccountManager,
        ignoredDirectories: [usageProbeDirectory],
      });
    }
    if (!state.grokUsageManager) {
      state.grokUsageManager = createGrokUsageManager({
        grokPath,
        accountManager: state.grokAccountManager,
        probeDirectory: path.join(app.getPath('userData'), 'grok-usage-probe'),
        storagePath: path.join(app.getPath('userData'), 'grok-usage-cache.json'),
        onSnapshot: (snapshot) => emitToAllWindows('openchamber:grok-usage-updated', snapshot),
        log: (message) => log.warn('[grok-code] usage:', message),
      });
      void state.grokUsageManager.prewarm().catch((error) => {
        log.warn('[grok-code] initial usage prewarm failed:', error);
      });
    }
    const browserMcpScript = app.isPackaged
      ? path.join(process.resourcesPath, 'browser-mcp', 'grok-browser-mcp.mjs')
      : [
          path.join(__dirname, 'resources', 'browser-mcp', 'grok-browser-mcp.mjs'),
          path.join(__dirname, '..', 'resources', 'browser-mcp', 'grok-browser-mcp.mjs'),
        ].find((candidate) => fs.existsSync(candidate));
    const bridge = createGrokOpenCodeBridge({
      defaultDirectory: process.env.OPENCHAMBER_OPENCODE_CWD,
      grokPath,
      grokEnv: state.grokAccountManager.activeEnvironment(),
      activeAccountId: state.grokAccountManager.list().currentId,
      stateFile: path.join(app.getPath('userData'), 'grok-code-sessions.json'),
      browserController: grokBrowserController,
      browserMcpScript,
      browserMcpExecutable: process.execPath,
      nativeSessionLoader: (sessionId) => requireGrokHistoryManager().preview(sessionId),
      log: (message) => log.info('[grok-code]', message),
    });
    const address = await bridge.listen({ port: 0, host: LOOPBACK_BIND_HOST });
    state.grokBridge = bridge;
    try {
      const nativeIndex = await state.grokHistoryManager.list({ limit: 500, force: true });
      await bridge.syncNativeSessions(nativeIndex.sessions, state.grokAccountManager.list().currentId);
    } catch (error) {
      log.warn('[grok-code] native Grok session index sync failed:', error);
    }
    process.env.OPENCODE_HOST = `http://${address.host}:${address.port}`;
    process.env.OPENCODE_SKIP_START = 'true';
    log.info('[grok-code] Grok ACP compatibility bridge ready', {
      host: address.host,
      port: address.port,
      directory: process.env.OPENCHAMBER_OPENCODE_CWD,
    });
  }
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: '连接到 Grok Code 服务器？',
    message: `连接到“${payload.label}”？`,
    detail:
      `这会把 ${payload.serverUrl} 添加为远程实例，并让本应用通过该服务器工作。` +
      '仅在你信任该服务器且由你本人发起连接时继续。',
    buttons: ['连接', '取消'],
    defaultId: 1,
    cancelId: 1,
  };
...
const handleInvoke = async (browserWindow, command, args = {}) => {
  switch (command) {
    ...
    case 'desktop_grok_accounts_list':
      return requireGrokAccountManager().list();

    case 'desktop_grok_account_create':
      return requireGrokAccountManager().create(args.label);

    case 'desktop_grok_account_rename':
      return requireGrokAccountManager().rename(String(args.id ?? ''), args.label);

    case 'desktop_grok_account_switch':
      return requireGrokAccountManager().switch(String(args.id ?? ''));

    case 'desktop_grok_account_login':
      return requireGrokAccountManager().login(
        String(args.id ?? ''),
        args.method === 'device' ? 'device' : 'oauth',
      );

    case 'desktop_grok_account_login_status':
      return requireGrokAccountManager().loginStatus(String(args.id ?? ''));

    case 'desktop_grok_account_logout':
      return requireGrokAccountManager().logout(String(args.id ?? ''));

    case 'desktop_grok_account_remove':
      return requireGrokAccountManager().remove(String(args.id ?? ''));

    case 'desktop_grok_history_list':
      return requireGrokHistoryManager().list({
        query: args.query,
        limit: args.limit,
        force: args.force === true,
      });

    case 'desktop_grok_history_preview':
      return requireGrokHistoryManager().preview(String(args.sessionId ?? ''));

    case 'desktop_grok_history_import': {
      if (!state.grokBridge) throw new Error('Grok 运行时尚未启动');
      const preview = await requireGrokHistoryManager().preview(String(args.sessionId ?? ''));
      return state.grokBridge.importNativeSession({
        nativeSessionId: preview.session.id,
        directory: preview.session.cwd,
        title: preview.session.title,
        blocks: preview.blocks,
        updatedAt: preview.session.updatedAt,
      });
    }

    case 'desktop_grok_usage_get':
      return requireGrokUsageManager().get({ force: args.force === true });
    ...
    case 'desktop_browser_register': {
      ...
      if (!contents || contents.isDestroyed() || contents.getType() !== 'webview') {
        throw new Error('Browser WebContents not found');
      }
      state.activeBrowserWebContentsId = wcId;
      return { registered: true, webContentsId: wcId, url: contents.getURL() };
    }

    case 'desktop_browser_unregister': {
      const wcId = Number.isFinite(args.webContentsId) ? Math.trunc(args.webContentsId) : null;
      if (wcId !== null && state.activeBrowserWebContentsId === wcId) {
        state.activeBrowserWebContentsId = null;
      }
      return { registered: false };
    }

    case 'desktop_browser_record_event': {
      const wcId = Number.isFinite(args.webContentsId) ? Math.trunc(args.webContentsId) : null;
      if (wcId === null || wcId !== state.activeBrowserWebContentsId) return { recorded: false };
      const eventName = String(args.name || '');
      if (eventName !== 'browser_navigate') return { recorded: false };
      const url = normalizeControllableBrowserUrl(args.arguments?.url);
      appendBrowserRecordingEvent(eventName, { url }, { url });
      return { recorded: Boolean(state.browserRecording) };
    }

    case 'desktop_browser_tool': {
      const name = String(args.name || '').trim();
      if (!name) throw new Error('Browser tool name is required');
      return await invokeBrowserTool(name, args.arguments && typeof args.arguments === 'object' ? args.arguments : {});
    }
    ...
    case 'desktop_check_for_updates': {
      ...
      if (shouldSkipDesktopUpdateCheck({
  ...
  return Menu.buildFromTemplate([
    {
      label: app.name,
      submenu: [
        { label: '关于 Grok Code', click: () => dispatchAction('about') },
        {
          label: '检查更新',
          click: () => dispatchCheckForUpdates(),
        },
        { type: 'separator' },
        { label: '设置', accelerator: 'Cmd+,', click: () => dispatchAction('settings') },
        { label: '重新加载界面', click: () => reloadMenuTargetWindow() },
        { label: '重启应用', click: () => relaunchFromMenu() },
        { label: '命令面板', accelerator: 'Cmd+P', click: () => dispatchAction('command-palette') },
        { type: 'separator' },
        { role: 'services' },
        { type: 'separator' },
        { role: 'hide' },
    ...
    },
    {
      label: '文件',
      submenu: [
        { label: '新建窗口', accelerator: 'Cmd+Shift+Alt+N', click: () => void handleInvoke(null, 'desktop_new_window') },
        { type: 'separator' },
        { label: '新建会话', accelerator: 'Cmd+N', click: () => dispatchAction('new-session') },
        { label: '新建工作树', accelerator: 'Cmd+Shift+N', click: () => dispatchAction('new-worktree-session') },
        ...
        { label: '新建迷你聊天', accelerator: 'Cmd+Alt+N', registerAccelerator: false, click: () => dispatchOpenMiniChat() },
        { type: 'separator' },
        { label: '添加工作区', click: () => dispatchAction('change-workspace') },
        { type: 'separator' },
        { role: 'close' },
      ],
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        { role: 'cut' },
        { label: '复制', accelerator: 'Cmd+C', click: () => handleCopyAction() },
        { label: '把所选内容加入聊天', accelerator: 'Cmd+L', registerAccelerator: false, click: () => dispatchAddSelectionToChat() },
        { role: 'paste' },
        { role: 'selectAll' },
      ],
    },
    {
      label: '视图',
      submenu: [
        { label: '显示或隐藏右侧栏', accelerator: 'Cmd+B', click: () => dispatchAction('toggle-right-sidebar') },
        { label: '打开 Git 侧栏', accelerator: 'Cmd+Shift+G', click: () => dispatchAction('open-right-sidebar-git') },
        { label: '打开文件侧栏', accelerator: 'Cmd+Shift+F', click: () => dispatchAction('open-right-sidebar-files') },
        { type: 'separator' },
        { label: '显示或隐藏终端', accelerator: 'Cmd+J', click: () => dispatchAction('toggle-terminal') },
        { label: '展开或收起终端', accelerator: 'Cmd+Shift+J', click: () => dispatchAction('toggle-terminal-expanded') },
        { type: 'separator' },
        { label: '浅色主题', click: () => dispatchAction('theme-light') },
        { label: '深色主题', click: () => dispatchAction('theme-dark') },
        { label: '跟随系统主题', click: () => dispatchAction('theme-system') },
        { type: 'separator' },
        { label: '显示或隐藏会话侧栏', accelerator: 'Cmd+Alt+L', click: () => dispatchAction('toggle-sidebar') },
        { label: '切换内存调试', accelerator: 'Cmd+Shift+D', click: () => dispatchAction('toggle-memory-debug') },
        { type: 'separator' },
    ...
    },
    {
      label: '帮助',
      submenu: [
        { label: '键盘快捷键', accelerator: 'Cmd+.', click: () => dispatchAction('help-dialog') },
        { label: '显示诊断信息', accelerator: 'Cmd+Shift+L', click: () => dispatchAction('download-logs') },
        { label: '开发者工具', accelerator: 'Cmd+Alt+I', click: () => openDevToolsForMenuTarget() },
        { type: 'separator' },
        { label: '清除缓存', click: () => void handleInvoke(null, 'desktop_clear_cache') },
        { type: 'separator' },
        { label: '报告问题', click: () => shell.openExternal(GITHUB_BUG_REPORT_URL) },
        { label: '提交功能建议', click: () => shell.openExternal(GITHUB_FEATURE_REQUEST_URL) },
        { type: 'separator' },
        { label: '加入 Discord', click: () => shell.openExternal(DISCORD_INVITE_URL) },
      ],
    },
  ]);
};
  ...
  return Menu.buildFromTemplate([
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        { label: '设置', accelerator: 'Ctrl+,', click: () => dispatchAction('settings') },
        { label: '重新加载界面', click: () => reloadMenuTargetWindow() },
        { label: '重启应用', click: () => relaunchFromMenu() },
        { label: '命令面板', accelerator: 'Ctrl+P', click: () => dispatchAction('command-palette') },
        { type: 'separator' },
        { role: 'quit' },
      ],
    },
    {
      label: '文件',
      submenu: [
        { label: '新建窗口', accelerator: 'Ctrl+Shift+Alt+N', click: () => void handleInvoke(null, 'desktop_new_window') },
        { type: 'separator' },
        { label: '新建会话', accelerator: 'Ctrl+N', click: () => dispatchAction('new-session') },
        { label: '新建工作树', accelerator: 'Ctrl+Shift+N', click: () => dispatchAction('new-worktree-session') },
        { type: 'separator' },
        { label: '添加工作区', click: () => dispatchAction('change-workspace') },
        { type: 'separator' },
        { role: 'quit' },
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        { role: 'cut' },
        { label: '复制', accelerator: 'Ctrl+C', click: () => handleCopyAction() },
        { label: '把所选内容加入聊天', accelerator: 'Ctrl+L', registerAccelerator: false, click: () => dispatchAddSelectionToChat() },
        { role: 'paste' },
        { role: 'selectAll' },
      ],
    },
    {
      ...
      submenu: [
        { role: 'reload' },
        { role: 'forceReload' },
        { label: '开发者工具', accelerator: 'Ctrl+Alt+I', click: () => openDevToolsForMenuTarget() },
        { type: 'separator' },
        { label: '显示或隐藏右侧栏', accelerator: 'Ctrl+B', click: () => dispatchAction('toggle-right-sidebar') },
        { label: '打开 Git 侧栏', accelerator: 'Ctrl+Shift+G', click: () => dispatchAction('open-right-sidebar-git') },
        { label: '打开文件侧栏', accelerator: 'Ctrl+Shift+F', click: () => dispatchAction('open-right-sidebar-files') },
        { type: 'separator' },
        { label: '显示或隐藏终端', accelerator: 'Ctrl+J', click: () => dispatchAction('toggle-terminal') },
        { label: '展开或收起终端', accelerator: 'Ctrl+Shift+J', click: () => dispatchAction('toggle-terminal-expanded') },
        { type: 'separator' },
        { label: '浅色主题', click: () => dispatchAction('theme-light') },
        { label: '深色主题', click: () => dispatchAction('theme-dark') },
        { label: '跟随系统主题', click: () => dispatchAction('theme-system') },
        { type: 'separator' },
        { label: '显示或隐藏会话侧栏', accelerator: 'Ctrl+Alt+L', click: () => dispatchAction('toggle-sidebar') },
        { label: '切换内存调试', accelerator: 'Ctrl+Shift+D', click: () => dispatchAction('toggle-memory-debug') },
        { type: 'separator' },
        { role: 'togglefullscreen' },
      ],
    },
    {
      label: '转到',
      submenu: [
        { label: '后退', accelerator: 'Ctrl+[', click: () => dispatchAction('go-back') },
        { label: '前进', accelerator: 'Ctrl+]', click: () => dispatchAction('go-forward') },
        { type: 'separator' },
        { label: '上一个会话', accelerator: 'Alt+Up', click: () => dispatchAction('previous-session') },
        { label: '下一个会话', accelerator: 'Alt+Down', click: () => dispatchAction('next-session') },
        { type: 'separator' },
        { label: '上一个项目', accelerator: 'Ctrl+Alt+Up', click: () => dispatchAction('previous-project') },
        { label: '下一个项目', accelerator: 'Ctrl+Alt+Down', click: () => dispatchAction('next-project') },
      ],
    },
    {
      label: '窗口',
    ...
    },
    {
      label: '帮助',
      submenu: [
        { label: '键盘快捷键', accelerator: 'Ctrl+.', click: () => dispatchAction('help-dialog') },
        { label: '显示诊断信息', accelerator: 'Ctrl+Shift+L', click: () => dispatchAction('download-logs') },
        { type: 'separator' },
        { label: '清除缓存', click: () => void handleInvoke(null, 'desktop_clear_cache') },
        { type: 'separator' },
        { label: '报告问题', click: () => shell.openExternal(GITHUB_BUG_REPORT_URL) },
...
app.whenReady().then(async () => {
  ...
  if (!GROK_CODE_MODE) {
    ...
    log.info('[grok-code] upstream OpenChamber auto-updater disabled');
  }
