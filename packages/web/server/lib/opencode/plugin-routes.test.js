...
describe('opencode plugin routes', () => {
  ...
  test('GET /api/config/plugins merges Grok-discovered plugins as read-only entries', async () => {
    const fetchImpl = mock(async () => new Response(JSON.stringify({
      plugins: [{
        ...
        enabled: true,
        provides: { skills: 1, mcpServers: 1 },
      }],
    }), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    app = createApp({
      buildOpenCodeUrl: (pathname) => `http://127.0.0.1:4097${pathname}`,
  ...
  test('GET /api/config/plugins preserves configured plugins when Grok discovery fails', async () => {
    plugins.createPluginEntry({ spec: 'configured-plugin', scope: 'user' }, projectDir);
    app = createApp({
      buildOpenCodeUrl: (pathname) => `http://127.0.0.1:4097${pathname}`,
    ...
    expect(response.body.entries).toHaveLength(1);
    expect(response.body.entries[0].spec).toBe('configured-plugin');
  });

  test('GET /api/config/plugins/grok-marketplace proxies search to the Grok bridge', async () => {
    const fetchImpl = mock(async (url) => {
      expect(String(url)).toContain('/grok/marketplace?query=browser');
    ...
    const response = await request(app)
      .get('/api/config/plugins/grok-marketplace')
      .query({ query: 'browser' })
      .expect(200);

    expect(response.body.plugins).toHaveLength(1);
    expect(response.body.plugins[0].spec).toBe('chrome-devtools@xAI Official');
  });

  test('POST /api/config/plugins/grok-install forwards explicit user installation', async () => {
    const fetchImpl = mock(async (url, init) => {
      expect(String(url)).toContain('/grok/plugins/install');
      expect(init.method).toBe('POST');
      expect(JSON.parse(init.body)).toEqual({ spec: 'chrome-devtools@xAI Official' });
      return new Response(JSON.stringify({ ok: true, output: { stdout: 'installed' } }), {
        status: 200,
    ...
    const response = await request(app)
      .post('/api/config/plugins/grok-install')
      .send({ spec: 'chrome-devtools@xAI Official' })
      .expect(200);

    expect(response.body.success).toBe(true);
    expect(response.body.spec).toBe('chrome-devtools@xAI Official');
  });
