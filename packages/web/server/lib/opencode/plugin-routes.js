...
export const registerPluginRoutes = (app, dependencies) => {
  ...
  const listGrokDiscoveredPlugins = async (directory) => {
    if (typeof buildOpenCodeUrl !== 'function') return [];
    try {
      const url = new URL(buildOpenCodeUrl('/grok/plugins', ''));
      ...
      return plugins.flatMap((plugin) => {
        ...
        return [{
          id: encodePluginId('grok', `${scope}:${name}:${pluginPath}`),
          spec: name,
          options: {
            ...(pluginPath ? { path: pluginPath } : {}),
            ...(plugin.provides && typeof plugin.provides === 'object'
      ...
      console.warn('[API:GET /api/config/plugins] Grok plugin discovery unavailable:', error?.message || error);
      return [];
    }
  };

  const requestGrokPluginEndpoint = async (pathname, directory, init = {}) => {
    if (typeof buildOpenCodeUrl !== 'function') {
      const error = new Error('Grok plugin bridge is unavailable');
      error.code = 'GROK_BRIDGE_UNAVAILABLE';
      throw error;
    }
    ...
    if (!response.ok) {
      const error = new Error(payload?.error || `Grok plugin request failed (${response.status})`);
  ...
  app.get('/api/config/plugins', async (req, res) => {
    try {
      ...
      const configuredEntries = listPluginEntries(directory);
      const discoveredEntries = await listGrokDiscoveredPlugins(directory);
      const configuredSpecs = new Set(configuredEntries.map((entry) => entry.spec));

      res.json({
        entries: [
          ...configuredEntries,
          ...discoveredEntries.filter((entry) => !configuredSpecs.has(entry.spec)),
        ],
  ...
  app.get('/api/config/plugins/grok-marketplace', async (req, res) => {
    try {
      const directory = await resolveDirectory(req, res);
      if (directory === null && res.headersSent) return;
      const query = typeof req.query?.query === 'string' ? req.query.query.trim() : '';
      const pathname = query
        ? `/grok/marketplace?query=${encodeURIComponent(query)}`
        : '/grok/marketplace';
      const payload = await requestGrokPluginEndpoint(pathname, directory);
      res.json({ plugins: Array.isArray(payload?.plugins) ? payload.plugins : [] });
    } catch (error) {
      console.error('[API:GET /api/config/plugins/grok-marketplace] Error:', error);
      res.status(Number(error?.statusCode) || 502).json({ error: error?.message || 'Failed to load Grok Marketplace' });
    }
  });

  app.post('/api/config/plugins/grok-install', async (req, res) => {
    try {
      const directory = await resolveDirectory(req, res);
      if (directory === null && res.headersSent) return;
      const spec = typeof req.body?.spec === 'string' ? req.body.spec.trim() : '';
      if (!spec || spec.length > 256 || /[\r\n\0]/.test(spec)) {
        res.status(400).json({ error: 'Invalid Grok plugin specification' });
        return;
      }
      const payload = await requestGrokPluginEndpoint('/grok/plugins/install', directory, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ spec }),
      });
      res.json({ success: true, spec, output: payload?.output ?? null });
    } catch (error) {
      console.error('[API:POST /api/config/plugins/grok-install] Error:', error);
      res.status(Number(error?.statusCode) || 500).json({ error: error?.message || 'Failed to install Grok plugin' });
    }
  });

  app.post('/api/config/plugins/grok-update', async (req, res) => {
    try {
      const directory = await resolveDirectory(req, res);
      if (directory === null && res.headersSent) return;
      const name = typeof req.body?.name === 'string' ? req.body.name.trim() : '';
      if (!name || name.length > 128 || /[\r\n\0]/.test(name)) {
        res.status(400).json({ error: 'Invalid Grok plugin name' });
        return;
      }
      const payload = await requestGrokPluginEndpoint('/grok/plugins/update', directory, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name }),
      });
      res.json({ success: true, name, output: payload?.output ?? null });
    } catch (error) {
      console.error('[API:POST /api/config/plugins/grok-update] Error:', error);
      res.status(Number(error?.statusCode) || 500).json({ error: error?.message || 'Failed to update Grok plugin' });
    }
  });

  app.post('/api/config/plugins/grok-uninstall', async (req, res) => {
    try {
      const directory = await resolveDirectory(req, res);
      if (directory === null && res.headersSent) return;
      const name = typeof req.body?.name === 'string' ? req.body.name.trim() : '';
      ...
      const payload = await requestGrokPluginEndpoint('/grok/plugins/uninstall', directory, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name }),
      });
      ...
      console.error('[API:POST /api/config/plugins/grok-uninstall] Error:', error);
      res.status(Number(error?.statusCode) || 500).json({ error: error?.message || 'Failed to uninstall Grok plugin' });
    }
  });
