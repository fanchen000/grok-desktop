...
export interface PluginEntry {
  ...
  /** Discovered from Grok's compatibility layer rather than owned by this config editor. */
  readOnly?: boolean;
  origin?: 'grok';
  enabled?: boolean;
}
...
export interface PluginsStore {
  ...
  loadGrokMarketplace: (query?: string) => Promise<GrokMarketplacePlugin[]>;
  installGrokMarketplacePlugin: (spec: string) => Promise<PluginMutationResult>;
  updateGrokPlugin: (name: string) => Promise<PluginMutationResult>;
  uninstallGrokPlugin: (name: string) => Promise<PluginMutationResult>;
      ...
      (set, get) => ({
        ...
        loadRegistryInfo: async (opts) => {
          const specs = dedupeSpecs(
            opts?.specs
            ?? get().entries.filter((entry) => entry.readOnly !== true).map((entry) => entry.spec),
          );
          if (specs.length === 0) {
        ...
        loadGrokMarketplace: async (query = '') => {
          const configDirectory = getConfigDirectory();
          const suffix = query.trim() ? `?query=${encodeURIComponent(query.trim())}` : '';
          try {
            const response = await runtimeFetch(
              buildPluginsUrl(`/api/config/plugins/grok-marketplace${suffix}`, configDirectory),
              { headers: buildDirectoryHeaders(configDirectory) },
            );
            const payload = await readJson<GrokMarketplaceResponse>(response).catch(
              (): GrokMarketplaceResponse => ({ plugins: [] }),
            );
            if (!response.ok) {
              throw new Error(payload.error || 'Failed to load Grok Marketplace');
            }
            return Array.isArray(payload.plugins) ? payload.plugins : [];
          } catch (error) {
            console.error('[PluginsStore] Failed to load Grok Marketplace:', error);
            return [];
          }
        },

        installGrokMarketplacePlugin: async (spec) => {
          const normalizedSpec = spec.trim();
          if (!normalizedSpec) return { ok: false };
          startConfigUpdate(normalizedSpec);
          try {
            const configDirectory = getConfigDirectory();
            const response = await runtimeFetch(
              buildPluginsUrl('/api/config/plugins/grok-install', configDirectory),
              {
                method: 'POST',
                headers: buildJsonHeaders(configDirectory),
                body: JSON.stringify({ spec: normalizedSpec }),
              },
            );
            const payload = await readJson<PluginMutationPayload | null>(response).catch(() => null);
            if (!response.ok) {
              throw new Error(payload?.error || 'Failed to install Grok plugin');
            }
            invalidatePluginCache(configDirectory);
            await get().loadPlugins({ force: true });
            return { ok: true, message: payload?.message };
          } catch (error) {
            console.error('[PluginsStore] Failed to install Grok Marketplace plugin:', error);
        ...
        updateGrokPlugin: async (name) => {
          const normalizedName = name.trim();
          if (!normalizedName) return { ok: false };
          startConfigUpdate(normalizedName);
          try {
            const configDirectory = getConfigDirectory();
            const response = await runtimeFetch(
              buildPluginsUrl('/api/config/plugins/grok-update', configDirectory),
              {
                method: 'POST',
                headers: buildJsonHeaders(configDirectory),
            ...
            const payload = await readJson<PluginMutationPayload | null>(response).catch(() => null);
            if (!response.ok) throw new Error(payload?.error || 'Failed to update Grok plugin');
            invalidatePluginCache(configDirectory);
            await get().loadPlugins({ force: true });
            return { ok: true, message: payload?.message };
          } catch (error) {
            console.error('[PluginsStore] Failed to update Grok plugin:', error);
            return { ok: false, message: error instanceof Error ? error.message : undefined };
          } finally {
            finishConfigUpdate();
          }
        ...
        uninstallGrokPlugin: async (name) => {
          const normalizedName = name.trim();
          if (!normalizedName) return { ok: false };
          startConfigUpdate(normalizedName);
          try {
            const configDirectory = getConfigDirectory();
            const response = await runtimeFetch(
              buildPluginsUrl('/api/config/plugins/grok-uninstall', configDirectory),
              {
                method: 'POST',
                headers: buildJsonHeaders(configDirectory),
                body: JSON.stringify({ name: normalizedName }),
            ...
            const payload = await readJson<PluginMutationPayload | null>(response).catch(() => null);
            if (!response.ok) throw new Error(payload?.error || 'Failed to uninstall Grok plugin');
            const configDirectoryAfter = getConfigDirectory();
            invalidatePluginCache(configDirectoryAfter);
            await get().loadPlugins({ force: true });
            return { ok: true, message: payload?.message };
          } catch (error) {
            console.error('[PluginsStore] Failed to uninstall Grok plugin:', error);
            return { ok: false, message: error instanceof Error ? error.message : undefined };
          } finally {
            finishConfigUpdate();
          }
        ...
        updateToLatest: async (id) => {
          ...
          if (!entry || entry.readOnly) return { ok: false };
        ...
        updateEntry: async (id, input) => {
          if (get().entries.find((plugin) => plugin.id === id)?.readOnly) {
            return { ok: false };
          }
