...
type DeleteTarget =
  ...
  | { kind: 'grok-entry'; id: string; label: string }
...
export const PluginsSidebar: React.FC<PluginsSidebarProps> = ({
  ...
  const { entries, files, selectedId, setSelected, deleteEntry, deleteFile, loadPlugins, updateGrokPlugin, uninstallGrokPlugin } =
    usePluginsStore(
      useShallow((s) => ({
        entries: s.entries,
        files: s.files,
        ...
        updateGrokPlugin: s.updateGrokPlugin,
        uninstallGrokPlugin: s.uninstallGrokPlugin,
      })),
    );
  ...
  const handleRefresh = React.useCallback(async () => {
    ...
    try {
      const loaded = await loadPlugins({ force: true });
      if (!loaded) throw new Error('Plugin discovery failed');
      await loadRegistryInfo({ force: true });
    } catch {
      toast.error(t('settings.plugins.toast.refreshFailed'));
    }
  }, [loadPlugins, loadRegistryInfo, t]);

  const handleGrokUpdate = React.useCallback(async (name: string) => {
    const result = await updateGrokPlugin(name);
    if (result.ok) {
      toast.success(t('settings.plugins.toast.updated'), { description: name });
    } else {
      toast.error(result.message || t('settings.plugins.grok.updateFailed'));
    }
  }, [t, updateGrokPlugin]);
  ...
  const handleDelete = React.useCallback(async () => {
    ...
    setIsDeleting(true);
    const result = deleteTarget.kind === 'entry'
      ? await deleteEntry(deleteTarget.id)
      : deleteTarget.kind === 'grok-entry'
        ? await uninstallGrokPlugin(deleteTarget.label)
    ...
    if (result.ok) {
      ...
      toast.error(
        result.message
        || t(deleteTarget.kind === 'grok-entry'
          ? 'settings.plugins.grok.uninstallFailed'
          : 'settings.plugins.sidebar.toast.deleteFailed'),
      );
    }
    setDeleteTarget(null);
    setIsDeleting(false);
  }, [deleteEntry, deleteFile, deleteTarget, t, uninstallGrokPlugin]);
  ...
  const renderEntry = (entry: PluginEntry) => {
    ...
    if (!entry.readOnly) {
      ...
      actions.push({
        label: t('settings.plugins.sidebar.actions.updateToLatest'),
        icon: 'arrow-up-s',
        onClick: () => void handleGrokUpdate(entry.spec),
      });
      actions.push({
        label: t('settings.common.actions.delete'),
        icon: 'delete-bin',
        destructive: true,
        onClick: () =>
          setDeleteTarget({ kind: 'grok-entry', id: entry.id, label: entry.spec }),
      });
    }
    return (
      <SettingsSidebarItem
        ...
        title={
          <span className="flex min-w-0 items-center gap-1.5">
            ...
            {!entry.readOnly && <RegistryBadge spec={entry.spec} />}
          </span>
        }
        metadata={
          entry.readOnly
