...
const DesktopBrowserPane: React.FC<DesktopBrowserPaneProps> = ({ initialUrl, directory, tabID }) => {
  ...
  const [isBrowserActionBusy, setIsBrowserActionBusy] = React.useState(false);
  ...
  React.useEffect(() => {
    ...
    const registerBrowser = () => {
      try {
        ...
        void invokeDesktopCommand('desktop_browser_register', { webContentsId }).catch(() => {});
    ...
    const recordNavigation = (url: string) => {
      try {
        const webContentsId = webview.getWebContentsId?.();
        if (!Number.isFinite(webContentsId) || !url || url === 'about:blank') return;
        void invokeDesktopCommand('desktop_browser_record_event', {
          webContentsId,
    ...
    return () => {
      try {
        ...
        if (Number.isFinite(webContentsId)) {
          void invokeDesktopCommand('desktop_browser_unregister', { webContentsId }).catch(() => {});
        }
      } catch { /* webview already destroyed */ }
      if (loadingTimerRef.current) clearTimeout(loadingTimerRef.current);
  ...
  const refreshBrowserAutomationStatus = React.useCallback(async () => {
    try {
      const status = await invokeDesktopCommand<{
        recording?: { id?: string } | null;
        recordings?: Array<{ id?: string }>;
      }>('desktop_browser_tool', { name: 'browser_status', arguments: {} });
      setIsRecording(Boolean(status?.recording));
      const latest = status?.recordings?.find((recording) => typeof recording.id === 'string')?.id;
      setLatestRecordingId(latest ?? status?.recording?.id ?? null);
    } catch {
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => void refreshBrowserAutomationStatus(), 400);
    return () => clearTimeout(timer);
  }, [currentUrl, refreshBrowserAutomationStatus]);

  const handleAppshot = React.useCallback(async () => {
    setIsBrowserActionBusy(true);
    try {
      await invokeDesktopCommand('desktop_browser_tool', {
        name: 'browser_appshot',
        arguments: { label: currentUrl },
      });
      toast.success(t('contextPanel.browser.appshotSaved'));
    } catch (error) {
      toast.error(t('contextPanel.browser.actionFailed'), {
        description: error instanceof Error ? error.message : undefined,
      });
    } finally {
      setIsBrowserActionBusy(false);
    }
  }, [currentUrl, t]);
  ...
  const handleRecordingToggle = React.useCallback(async () => {
    ...
    try {
      if (isRecording) {
        const result = await invokeDesktopCommand<{ id?: string }>('desktop_browser_tool', {
          name: 'browser_record_stop',
          arguments: {},
        });
        setIsRecording(false);
        if (result?.id) setLatestRecordingId(result.id);
        toast.success(t('contextPanel.browser.recordingStopped'));
      } else {
        const result = await invokeDesktopCommand<{ id?: string }>('desktop_browser_tool', {
        ...
        setIsRecording(true);
        if (result?.id) setLatestRecordingId(result.id);
        toast.success(t('contextPanel.browser.recordingStarted'));
      }
    } catch (error) {
      toast.error(t('contextPanel.browser.actionFailed'), {
        description: error instanceof Error ? error.message : undefined,
      });
      await refreshBrowserAutomationStatus();
    } finally {
      setIsBrowserActionBusy(false);
    }
  }, [currentUrl, isRecording, refreshBrowserAutomationStatus, t]);

  const handleReplayLatest = React.useCallback(async () => {
    let recordingId = latestRecordingId;
    if (!recordingId) {
      await refreshBrowserAutomationStatus();
      try {
        const status = await invokeDesktopCommand<{ recordings?: Array<{ id?: string }> }>(
          'desktop_browser_tool',
          { name: 'browser_status', arguments: {} },
        );
        recordingId = status?.recordings?.find((recording) => typeof recording.id === 'string')?.id ?? null;
      } catch {
        // The replay button remains a no-op when the desktop controller is
        // unavailable or no recording has been persisted yet.
      }
    }
    ...
    try {
      ...
      toast.success(t('contextPanel.browser.replayCompleted'));
    } catch (error) {
      toast.error(t('contextPanel.browser.actionFailed'), {
        description: error instanceof Error ? error.message : undefined,
      });
  ...
  }, [latestRecordingId, refreshBrowserAutomationStatus, t]);
    ...
    <div className="absolute inset-0 flex flex-col bg-background">
      <div className="flex items-center gap-1 border-b border-border bg-[var(--surface-background)] px-2 py-1">
        ...
        <Button
          ...
          size="sm"
          className="h-7 w-7 p-0"
          onClick={() => void handleAppshot()}
          disabled={isBrowserActionBusy || !currentUrl || currentUrl === 'about:blank'}
          title={t('contextPanel.browser.appshot')}
          aria-label={t('contextPanel.browser.appshot')}
        >
        ...
        <Button
          ...
          disabled={isBrowserActionBusy || !currentUrl || currentUrl === 'about:blank'}
          title={t(isRecording ? 'contextPanel.browser.recordStop' : 'contextPanel.browser.recordStart')}
          aria-label={t(isRecording ? 'contextPanel.browser.recordStop' : 'contextPanel.browser.recordStart')}
        >
          <Icon
            name={isRecording ? 'stop' : 'record-circle'}
            className={cn('h-3.5 w-3.5', isRecording && 'text-[var(--status-error)]')}
          />
        </Button>
        <Button
          type="button"
          ...
          size="sm"
          className="h-7 w-7 p-0"
          onClick={() => void handleReplayLatest()}
          disabled={isBrowserActionBusy || isRecording || !latestRecordingId}
          title={t('contextPanel.browser.replayLatest')}
          aria-label={t('contextPanel.browser.replayLatest')}
        >
