...
export const Header: React.FC<HeaderProps> = ({
  ...
  const localInstanceLabel = t('desktopHostSwitcher.instance.local');
  const fallbackInstanceLabel = t('desktopHostSwitcher.instance.fallback');
  const [currentInstanceLabel, setCurrentInstanceLabel] = React.useState(() => localInstanceLabel);
  ...
  const refreshCurrentInstanceLabel = React.useCallback(async () => {
    ...
    try {
      if (isDesktopLocalOriginActive()) {
        setCurrentInstanceLabel(localInstanceLabel);
      ...
      setCurrentInstanceLabel(fallbackInstanceLabel);
    } catch {
      setCurrentInstanceLabel(localInstanceLabel);
      setCurrentInstanceIsLocal(true);
    }
  }, [fallbackInstanceLabel, isDesktopApp, localInstanceLabel]);
  ...
  const renderDesktop = () => (
    <div
      ...
      className={cn(
        'grok-codex-header app-region-drag relative flex h-12 select-none items-center pr-3',
