...
- Grok Code desktop inserts `GrokNativeHistorySection` between the sidebar controls and project zones. It shows a directly readable, per-account subscription usage meter from the privileged desktop service (used percentage, remaining percentage, reset time, stale/error state) and local native Grok history. Usage refresh is cached in the main process, polls only while the sidebar is visible, and never reads credentials in the renderer.
