...
const DEFAULT_DICTIONARY = zhCnDict;
const dictionaries = new Map<Locale, I18nDictionary>([
  ['en', enDict],
  ['zh-CN', zhCnDict],
]);
...
export const useI18nStore = create<I18nState>()((set, get) => ({
  ...
  setLocale: (locale) => {
    ...
    void loadDictionary(locale).then((dictionary) => {
      ...
      if (get().locale === locale) {
        set({ dictionary: DEFAULT_DICTIONARY, loadingLocale: null });
      }
    });
  },
}));
...
export function formatMessage(dictionary: I18nDictionary, key: I18nKey, params?: I18nParams): string {
  const source = dictionary[key] ?? enDict[key] ?? key;
  const template = source
    .replaceAll('OpenChamber', 'Grok Code')
    .replaceAll('OpenCode', 'Grok');
  if (!params) {
    return template;
  }
