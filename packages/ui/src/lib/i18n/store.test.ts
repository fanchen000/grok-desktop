...
describe('i18n store', () => {
  ...
  test('starts with the Simplified Chinese dictionary', () => {
    expect(useI18nStore.getState().locale).toBe('zh-CN');
    expect(useI18nStore.getState().dictionary['layout.mainTab.chat']).toBe('聊天');
  });

  test('uses Grok Code product branding in translated UI copy', () => {
    expect(formatMessage(defaultDictionary, 'settings.window.description')).toBe('Grok Code 设置窗口。');
    expect(formatMessage(defaultDictionary, 'openCodeStatusDialog.title')).toBe('Grok 状态');
  });
});
